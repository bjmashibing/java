...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	//静态初始化模块，在整个容器创建过程中只执行一次
	static {
		// Eagerly load the ContextClosedEvent class to avoid weird classloader issues
		// on application shutdown in WebLogic 8.1. (Reported by Dustin Woods.)
		//加载IOC容器关闭事件类，为了避免应用程序出现类加载异常问题
	...
	protected ResourcePatternResolver getResourcePatternResolver() {
		//AbstractApplicationContext继承DefaultResourceLoader,因此是一个资源加载器
		//Spring 资源加载器的getResource方法用于载入资源
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			//把创建好的MessageSource注册在容器中，当能获取到国际化配置文件的值的时候，就可以自动注入MessageSource
